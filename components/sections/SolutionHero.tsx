...
export default function SolutionHero({ content }: { content: SolutionHeroContent }) {
  ...
  return (
    <section className="relative flex min-h-screen items-end overflow-hidden bg-navy">
      ...
      <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-navy via-navy/45 to-navy/20" />
      ...
      <div className="relative mx-auto w-full max-w-content px-6 pb-20 pt-40 sm:pb-24">
        <motion.div
          ...
          className="max-w-3xl"
        >
          <p className="font-heading text-base font-bold text-accentYellow">{content.eyebrow}</p>
          <h1 className="mt-4 font-heading text-4xl font-bold leading-[1.08] text-white sm:text-5xl lg:text-[56px]">
            {content.title}
          </h1>
          <p className="mt-6 max-w-2xl text-base leading-relaxed text-white/85 sm:text-lg">
            {content.subtitle}
          </p>
          {content.note && <p className="mt-4 max-w-2xl text-sm text-white/60">{content.note}</p>}
